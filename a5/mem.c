 ...
 * MODIFIED BY: Richard Lee, Section 2
 * *****************************************************************************/
...
void* Mem_Alloc(int size)
{
  if (size <= 0) {
    ...
    /* Extra size needed to make the allocation size a multiple of 4 */
  int padsize;
  /* Size of the block without the block header */
  int real_size;
  /* Total Size of the block with the block header */
  int alloc_size;
  /* A storage variable */
  int block_size;
  ...
  block_header *temp;
  /* Storage variable */
  block_header *store = NULL;
  block_header *bestBlock = NULL;
  /* Variable for pointer arithmetic */
  char *actual_size;
  int maxBlockSize = 2147483647;
  /* Process of making the allocation size a multiple of 4 */
  padsize = size % 4;
  padsize = (4 - padsize) % 4;
  real_size = size + padsize;
  
  /* Computing total size of the memory allocation needed */
  alloc_size = size + padsize + (int)sizeof(block_header);
  /* Sets the block_head pointer to the start of the linked list */
  temp = list_head;
  ...
  while(temp != NULL) {
    ...
    block_size = temp->size_status;

    /* Checks if the current block is free and big enough for allocation */
    if(block_size%2 == 0 && block_size >= alloc_size) {
  ...
  if(bestBlock == NULL) {
    ...
    /* If the remaining block size is not large enough to make a block
       * and a block header, return a block with the extra size
       */
    if(bestBlock->size_status - alloc_size < 12) {
      bestBlock->size_status = bestBlock->size_status + 1;
      return (bestBlock + 1);
    } /* Else split into 2 blocks */
    else {
      actual_size = (char *)bestBlock;
      actual_size += alloc_size;
      bestBlock->size_status = real_size + 1;
      bestBlock->next = (block_header *)actual_size;
      bestBlock->next->size_status = block_size - alloc_size;
      bestBlock->next->next = store;
      return (bestBlock + 1);
    }
  }
